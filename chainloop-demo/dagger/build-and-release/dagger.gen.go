...
import (
	...
	"github.com/chainloop-dev/integration-demo/chainloop-demo/dagger/build-and-release/querybuilder"
)
...
func (r *Golang) Base(version string) *Golang {
	q := r.q.Select("base")
...
func (r *Golang) Build(args []string, opts ...GolangBuildOpts) *Directory {
	q := r.q.Select("build")
	for i := len(opts) - 1; i >= 0; i-- {
...
// GolangBuildRemoteOpts contains options for Golang.BuildRemote
type GolangBuildRemoteOpts struct {
	Arch string
...
func (r *Golang) BuildRemote(remote string, ref string, module string, opts ...GolangBuildRemoteOpts) *Directory {
	q := r.q.Select("buildRemote")
	for i := len(opts) - 1; i >= 0; i-- {
		// `arch` optional argument
		if !querybuilder.IsZeroValue(opts[i].Arch) {
			q = q.Arg("arch", opts[i].Arch)
...
// Lint the Go project
func (r *Golang) GolangciLint(ctx context.Context) (string, error) {
...
func (r *Golang) Project() *Directory {
...
// Test the Go project
func (r *Golang) Test(ctx context.Context, args []string) (string, error) {
	if r.test != nil {
		return *r.test, nil
	}
	q := r.q.Select("test")
...
func (r *Golang) WithContainer(ctr *Container) *Golang {
...
func (r *Golang) WithProject(dir *Directory) *Golang {
	assertNotNil("dir", dir)
	q := r.q.Select("withProject")
	q = q.Arg("dir", dir)
...
// GolangOpts contains options for Client.Golang
...
func (r *Client) Golang(opts ...GolangOpts) *Golang {
	q := r.q.Select("golang")
	for i := len(opts) - 1; i >= 0; i-- {
		...
		// `proj` optional argument
		if !querybuilder.IsZeroValue(opts[i].Proj) {
			q = q.Arg("proj", opts[i].Proj)
		}
	}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "BuildAndRelease":
		...
		case "Build":
			...
			var proj *Directory
			if inputArgs["proj"] != nil {
				err = json.Unmarshal([]byte(inputArgs["proj"]), &proj)
				if err != nil {
